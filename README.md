...
- Configure Vercel to use the repository's `vercel.json` settings:
  ...
  - Build command: conditional Convex deploy when `CONVEX_DEPLOY_KEY` is present, otherwise a
...
- Without `CONVEX_DEPLOY_KEY`, Vercel can build the static frontend, but that deploy has not deployed
  Convex functions and is not launch-ready.
